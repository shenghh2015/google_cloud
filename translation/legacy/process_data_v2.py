...
def main():
    ...
    if use_missed:
        import numpy as np
        # missed_ids = np.load(os.path.expanduser('~/data/missed_ids.npy')).tolist()
        missed_ids = np.load(os.path.expanduser('~/data/missed_ids2.npy')).tolist()
        output_path = os.path.join(output_dir, f'data.{uid}.{dataset_name}.{today}_missed.pydict')
        for data in pydict_data_:
            prompt_id = data['id']
            if prompt_id in missed_ids:
